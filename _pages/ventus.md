...
This project evaluated a new piezoelectric vacuum pump (the "Ventus" pump) as a candidate replacement for a legacy diaphragm pump used in an industrial printhead. My role was to do the mechanical and electrical integration work needed to make the new pump operate inside an existing printer chassis, have it communicate with a host PC running PID control software, and tune its closed-loop PID controller to respond well in response to regular printer pneumatic purges.

I then replicated the setup 4 times to be used for accelerated life and reliability testing. 
...
The new pump came as a "Smart Pump Module" (SPM) which was a small board that includes the pump, drive electronics, and a serial interface in one package. I needed to mount the board in the same location on the printer as the legacy pump while interfacing with the printer's existing pneumatic plumbing.

I designed a **custom mounting bracket** that:

- Bolts to the printer chassis at the legacy pump's existing mounting points. This resulted in no permanent modifications to hardware in case the printers needed to be reused.
- Held the SPM with M2 hardware vertically to not allow condensation to ruin the pump
- Included a **neoprene isolation pad** between the SPM and the bracket to prevent shorts to the metal chassis and dampen the pump's mechanical vibrations
- Routes the SPM's pneumatic ports to align cleanly with the printer's existing tubing

I also specified replacing the SPM's stock thin O-rings with same-size **Viton** rings, because the originals were susceptible to ozone attack.
...
The smart pump module needed three external connections: power, a serial connection to the PC control app, and a 0–3.3 V analog feedback signal from an external pressure sensor for closed-loop vacuum regulation.

### Power and Communications Harness

I made a custom harness modified from an FTDI cable that breaks out the SPM's pins into:

- **Power and ground** to the host PC
- **UART (RX/TX) routed from an FTDI cable** to the lab PC, which presents the SPM as a virtual COM port for the manufacturer's control app
...
While the SPM has an onboard vacuum sensor, it is not sensitive enough for the small vacuum range that we were trying to control. Therefore, we decided to use 4 of our Alpha instruments differential pressure sensors that output 0-10V. 
The SPM's analog input accepts a 0–3.3V signal. So the output of the pressure sensor was scaled down by a factor of 1/3 using a voltage divider. However, there were some issues that needed to be fixed before they could be used with the pump module. 
...
To fix this, I made a signal conditioning board that connected directly on top of the pressure sensor. It consisted of the following:
...
The pull down resistor was actually very important for the safety of the SPM electrical hardware. The process value that was being measured and controlled was a vacuum, and the intermittent disturbances to be controlled for were ink "purges" where the vacuum region is positively pressurized to clean the print head of debris. When the vacuum sensor reads a positive pressure rather than a vacuum, the output signal goes negative, and this raw negative signal fed into the Op-amp caused the output to rise to the supply voltage (+24V). This mistake killed at least one SPM board. 

I worked with an EE colleague to package the circuit on small breadboards sized to fit within the footprint of the pressure sensor itself. Power was tapped from the sensor's own supply via spade connectors, and the conditioned output broke out to a 2-pin screw terminal so the harness could connect securely without solder joints in the field.
...
I then tested the system's response to a disturbance by pressurizing the volume and measuring the pump's ability to restore the vacuum. 
